...
const enforceHttps = (req, res, next) => {
    ...
    if (protocol !== 'https') {
        logger.warn(`⚠️ HTTP request detected (protocol: ${protocol}). Redirecting to HTTPS.`);
...
const addResponseIntegrityHeader = (req, res, next) => {
    ...
    const isSensitiveEndpoint = req.path.match(/\/(auth|admin|profile|quiz|score|note)\//i);
