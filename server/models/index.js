...
function buildSslConfig() {
  ...
  try {
    ...
    logger.warn("⚠️ تعذر قراءة شهادة CA، سيتم استخدام شهادات النظام.", {
      error: err.message,
    });
  }
...
if (!process.env.DB_PASSWORD && process.env.NODE_ENV === "production") {
  logger.error(
    "🚨 SECURITY ERROR: DB_PASSWORD environment variable is NOT set in production!",
  );
  logger.error(
    "Please set DB_PASSWORD in your production environment before deploying.",
  );
  process.exit(1);
}
