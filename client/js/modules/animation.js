...
function ensureScrollTriggerLoaded(timeout = 3000) {
  ...
  return new Promise((resolve) => {
    ...
    if ("requestIdleCallback" in window)
      requestIdleCallback(() => document.head.appendChild(s), {
        timeout: 2000,
      });
...
export async function initAnimations(perfOverride) {
  ...
  try {
    const perf =
      perfOverride || (await getDevicePerformanceTier({ skipFPSTest: true }));
    try {
...
export function animateCards(targets, options = {}) {
  ...
  const actualStagger =
    currentTier === "medium" ? Math.max(baseStagger, 0.12) : baseStagger;
...
export function animateSheetClose(sheetEl, overlayEl, onComplete) {
  ...
  tl.to(sheetEl, {
    ...
    duration: dur(0.3),
    ease: "power2.in",
  }).to(overlayEl, { opacity: 0, duration: dur(0.2), ease: "none" }, "-=0.15");
...
export function animateModalClose(modalEl, dialogEl, onComplete) {
  ...
  tl.to(dialogEl, {
    ...
    duration: dur(0.25),
    ease: "power2.in",
  }).to(modalEl, { opacity: 0, duration: dur(0.15), ease: "none" }, "-=0.1");
