...
function waitForSheetClose(contentEl) {
  return new Promise((resolve) => {
    ...
    setTimeout(finish, SHEET_CLOSE_MS + 60);
  });
}
...
function getCurrentMainSheetNodes() {
  ...
  if (notesContent?.classList.contains("active"))
    return getMainSheetNodes("notes");

  const examsContent = document.getElementById("tree-content");
  if (examsContent?.classList.contains("active"))
    return getMainSheetNodes("exams");

  return getMainSheetNodes(
    state.currentViewMode === "notes" ? "notes" : "exams",
  );
}
...
const onModalTouchMove = (e) => {
  ...
  const currentY =
    e.touches && e.touches.length > 0 ? e.touches[0].clientY : modalTouchStartY;
  const deltaY = currentY - modalTouchStartY;
  const canScrollInside = canScrollWithinActiveSheet(
    target,
    activeSheet,
    deltaY,
  );
...
function setModalTouchScrollLock(enabled) {
  ...
  if (enabled) {
    document.addEventListener("touchstart", onModalTouchStart, {
      capture: true,
      passive: true,
    });
    document.addEventListener("touchmove", onModalTouchMove, {
      capture: true,
      passive: false,
    });
  } else {
    document.removeEventListener("touchstart", onModalTouchStart, {
      capture: true,
    });
    document.removeEventListener("touchmove", onModalTouchMove, {
      capture: true,
    });
  }
}
...
function bindTreeScrollIndicator() {
  ...
  treeScrollIndicatorObserver = new MutationObserver(
    scheduleTreeScrollIndicatorUpdate,
  );
  if (historyTree) {
    treeScrollIndicatorObserver.observe(historyTree, {
      childList: true,
      subtree: true,
      attributes: true,
    });
  ...
  if (subjectFilters) {
    treeScrollIndicatorObserver.observe(subjectFilters, {
      childList: true,
      subtree: true,
      attributes: true,
    });
...
export function updateDockUI(activeId) {
  ...
  if (activeBtn) {
    ...
    if (activeId === "exams") activeBtn.classList.add("text-blue-600");
      ...
      activeBtn.classList.add("text-purple-600");
...
export function openBottomSheet(mode = null) {
  ...
  const otherNodes = getMainSheetNodes(
    nodes.mode === "notes" ? "exams" : "notes",
  );
...
export function closeBottomSheet() {
  ...
  const sheetsToClose = [
    getMainSheetNodes("exams"),
    getMainSheetNodes("notes"),
  ];
  ...
  sheetsToClose.forEach((nodes) => {
    ...
    if (sheet) {
      ...
      waitForSheetClose(content).then(() => {
        const latestContent = document.getElementById(nodes.contentId);
        if (!latestContent?.classList.contains("active"))
