...
export async function updateExistingNote(
  ...
  const noteId =
    state.allNotes[state.editingNoteIndex].id ||
    state.allNotes[state.editingNoteIndex].config?.id;
  ...
  try {
    ...
    state.allNotes[state.editingNoteIndex].config.description = document
      .getElementById("new-n-desc")
      .value.trim();
    state.allNotes[state.editingNoteIndex].config.type =
