...
export function openCreateSection() {
  ...
  document.getElementById("builder-subtitle").innerText =
...
export function goToBuilderStep2() {
  ...
  if (!title) {
    showAlert("⚠️ يجب إدخال عنوان للامتحان للبدء!", "warning");
    return;
  }
  if (!subject) {
    showAlert("⚠️ يجب اختيار مادة للامتحان للبدء!", "warning");
    return;
  }

  const closingMsg =
    document.getElementById("new-q-closing-msg").value.trim() ||
    "شكراً لمشاركتك في الاختبار!";
  ...
  if (!state.quizDraft) {
    state.quizDraft = {
      config: {
        ...
        timeLimit:
          (parseInt(document.getElementById("new-q-time").value) || 30) * 60,
        streakGoal: 3,
        language: "ar",
    ...
    state.quizDraft.config.timeLimit =
      (parseInt(document.getElementById("new-q-time").value) || 30) * 60;
    state.quizDraft.config.description =
      document.getElementById("new-q-desc").value.trim() || "";
  }
  ...
  document.getElementById("builder-subtitle").innerText =
    "الخطوة 2: إضافة الأسئلة وتحديد الإجابات";
...
export function renderBuilderQuestion() {
  ...
  document.getElementById("b-total-num").innerText =
    state.quizDraft.questions.length;
...
export function setBuilderCorrectOption(idx) {
  ...
  state.quizDraft.questions[state.bCurrentQIndex].answerOptions.forEach(
    (o, i) => (o.isCorrect = i === idx),
  );
  renderBuilderQuestion();
}
...
export function removeBuilderOption(idx) {
  ...
  if (q.answerOptions.length > 2) {
    ...
    if (!q.answerOptions.some((o) => o.isCorrect))
      q.answerOptions[0].isCorrect = true;
    renderBuilderQuestion();
  } else {
 ...
 */
export async function saveBuiltQuiz(
  renderHistoryTree,
  renderEditTree,
  renderDashboard,
) {
  ...
  for (let i = 0; i < state.quizDraft.questions.length; i++) {
    ...
    if (!q.question.trim()) {
      showAlert(
        `⚠️ السؤال رقم ${i + 1} فارغ! يرجى كتابته قبل الحفظ.`,
        "warning",
      );
    ...
    for (let j = 0; j < q.answerOptions.length; j++) {
      if (!q.answerOptions[j].text.trim()) {
        showAlert(
          `⚠️ الخيار رقم ${j + 1} في السؤال رقم ${i + 1} فارغ!`,
          "warning",
        );
        state.bCurrentQIndex = i;
        renderBuilderQuestion();
...
export function loadQuizIntoBuilder(index) {
  ...
  document.getElementById("new-q-closing-msg").value =
    qData.config.closingMessage || "";
 ...
 * @param {Function} renderDashboard — دالة رسم لوحة التحكم
 */
export async function updateExistingQuiz(
  index,
  renderHistoryTree,
  renderEditTree,
  renderDashboard,
...
export function reshuffleImportedAnswers() {
  ...
  if (
    !state.quizDraft ||
    !state.quizDraft.questions ||
    !state.quizDraft.questions.length
  ) {
    showAlert(
      "⚠️ لا توجد أسئلة لإعادة توزيع الإجابات بعد. استورد ملف أولاً.",
      "warning",
    );
    return;
  }
...
export async function handleImportFileChange(event) {
  ...
  try {
    ...
    if (!parsedQuestions.length)
      throw new Error("لم يتم العثور على أسئلة بصيغة أيكن.");

    applyImportedQuestions(parsedQuestions);
    showAlert(
      `✅ تم استيراد ${parsedQuestions.length} سؤالاً مع خلط الإجابات وضبط تكرار الحروف.`,
    );
  } catch (err) {
  ...
  questions.forEach((q) => {
    ...
    if (letter === lastLetter) {
      ...
      if (repeatCount > maxRepeat) {
        const swapIdx = q.answerOptions.findIndex(
          (_, idx) => idx !== correctIdx,
        );
        if (swapIdx >= 0) {
          [q.answerOptions[correctIdx], q.answerOptions[swapIdx]] = [
            q.answerOptions[swapIdx],
            q.answerOptions[correctIdx],
          ];
          correctIdx = swapIdx;
