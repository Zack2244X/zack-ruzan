...
code = code.replace(
  /if \(state\.userAnswers\[state\.currentQuestionIndex\] !== null\) \{\s*const \{ selectedIndex, isCorrect \} = state\.userAnswers\[state\.currentQuestionIndex\];\s*disableOptions\(\);\s*if \(index === selectedIndex\) optionEl\.classList\.add\('selected', isCorrect \? 'correct-answer' : 'incorrect-answer'\);\s*if \(option\.isCorrect\) optionEl\.classList\.add\('correct-answer'\);\s*\}/g,
...
code = code.replace(
  /if \(state\.userAnswers\[state\.currentQuestionIndex\] !== null\) \{\s*const \{ isCorrect, rationale, feedbackMessage \} = state\.userAnswers\[state\.currentQuestionIndex\];\s*showFeedback\(isCorrect, rationale, feedbackMessage\);\s*\}/g,
  `// Feedback disabled in normal running`,
);

// Remove hiding feedback
code = code.replace(
  /if \(state\.userAnswers\[state\.currentQuestionIndex\] === null\) hideFeedback\(\);/g,
  `hideFeedback();`,
);
