...
const modFiles = fs
  .readdirSync(modDir)
  .filter(
    (f) => f.endsWith(".js") && !f.includes(".min") && !f.includes(".bundle"),
  )
...
esbuild.buildSync({
  entryPoints: [path.join(jsDir, "app.admin.entry.js")],
...
esbuild.buildSync({
  entryPoints: [path.join(jsDir, "app.features.entry.js")],
